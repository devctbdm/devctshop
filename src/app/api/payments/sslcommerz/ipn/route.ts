...
export async function POST(request: Request) {
  ...
  try {
    ...
    } else if (data.tran_id && ["FAILED", "CANCELLED", "EXPIRED", "UNATTEMPTED"].includes(data.status ?? "")) {
      await markPaymentOutcome(data.tran_id, data.status === "CANCELLED" ? "cancelled" : "failed", data)
    }
