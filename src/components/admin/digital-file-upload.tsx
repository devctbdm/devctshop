    ...
    try {
      ...
      for (const file of Array.from(selected)) {
        ...
        if (file.size <= 0 || file.size > MAX_SIZE)
          throw new Error(`${file.name}: maximum size is 500 MB.`);
    ...
    <div className="space-y-3 sm:col-span-2">
      <div className="rounded-xl border border-dashed bg-muted/20 p-5">
        <label className="flex cursor-pointer flex-col items-center justify-center gap-2 text-center">
          <span className="flex size-10 items-center justify-center rounded-lg bg-background shadow-sm">
            <UploadCloudIcon className="size-5 text-muted-foreground" />
          </span>
          <span className="text-sm font-medium">Upload source-code ZIP</span>
          <span className="text-xs text-muted-foreground">
            Private Cloudflare R2 storage · ZIP only · up to 500 MB
          </span>
          <input
            type="file"
            multiple
        ...
        {uploading ? (
          <p className="mt-3 flex items-center justify-center gap-2 text-xs text-muted-foreground">
            ...
            Uploading to private R2 storage…
          </p>
        ) : null}
        {error ? (
          ...
          >
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="text-xs text-muted-foreground">
                {formatBytes(file.sizeBytes)} · private R2 file
              </p>
            </div>
            <button
              type="button"
      ...
      <input type="hidden" name="digitalFiles" value={JSON.stringify(files)} />
    </div>
  );
}
