...
function toAsset(info: Record<string, unknown>): CloudinaryAsset | null {
  ...
  return {
    ...
    format: typeof info.format === "string" ? info.format : "",
  };
}
  ...
  if (!cloudName || !apiKey)
    return (
      <p role="alert" className="text-xs text-destructive">
        Cloudinary is not configured. Restart the dev server after updating
        .env.
      </p>
    );
  return (
    <div className="space-y-2">
      <CldUploadWidget
        ...
        onSuccess={(result) => {
          ...
          const assets = Array.isArray(info)
            ? info
                .map((item) => toAsset(item as Record<string, unknown>))
                .filter((item): item is CloudinaryAsset => Boolean(item))
            : [toAsset(info as Record<string, unknown>)].filter(
                (item): item is CloudinaryAsset => Boolean(item),
              );
          if (assets.length) {
            setError(null);
            onUpload(assets);
      ...
      >
        {({ open, isLoading }) => (
          ...
          >
            ...
            {isLoading ? "Preparing upload…" : label}
          </Button>
        )}
      </CldUploadWidget>
      {error ? (
